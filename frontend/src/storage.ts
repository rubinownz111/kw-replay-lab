import type {SavedReplay} from './types';
const MAX_BYTES=256*1024*1024, MAX_ITEMS=50;
async function open(): Promise<IDBDatabase> {
 return new Promise((resolve,reject)=>{const request=indexedDB.open('kw-replay-lab-v1',1);request.onupgradeneeded=()=>request.result.createObjectStore('replays',{keyPath:'hash'});request.onsuccess=()=>resolve(request.result);request.onerror=()=>reject(new Error('Browser storage is unavailable. You can still analyze and export.'));});
}
